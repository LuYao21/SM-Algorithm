...
public class SecurityTestAll {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SM2KeyVO sm2KeyVO = generateSM2Key();//产生密钥对，generateSM2Key()就定义在此文件中。
        System.out.println("公钥:" + sm2KeyVO.getPubHexInSoft());//获得16进制的软件公钥，65btye，520bit,前8位为0x04说明公钥是非压缩格式
        System.out.println("私钥:" + sm2KeyVO.getPriHexInSoft());//获得16进制的软件私钥，生成256bit
        //数据加密
        System.out.println("--测试加密开始--");
        String src = "I Love You";
        System.out.println("原文UTF-8转hex:" + Util.byteToHex(src.getBytes()));
        String SM2Enc = SM2Enc(sm2KeyVO.getPubHexInSoft(), src);//SM2Enc方法在本源码中，对输入的数据进行了类型的转换
        System.out.println("加密:");
        System.out.println("密文:" + SM2Enc);
        String SM2Dec = SM2Dec(sm2KeyVO.getPriHexInSoft(), SM2Enc);////SM2Dec方法在本源码中，对输入的数据进行了类型的转换
        System.out.println("解密:" + SM2Dec);
        System.out.println("--测试加密结束--");

        System.out.println("--测试SM2签名--");
        System.out.println("原文hex:" + Util.byteToHex(src.getBytes()));//原文由字符串转换到16进制
